#include "./include/cyrobot_monitor/TabWidget/MyTabWidget.h"

#include "./include/cyrobot_monitor/TabWidget/MyTabBar.h"
#include "./include/cyrobot_monitor/TabWidget/MyTabPage.h"
    ...
    connect(bar, &MyTabBar::beginDragOut, this, [this, bar](int index) {
        ...
        connect(drag, &QDrag::destroyed, this, [=] {
            ...
            if (!bar->contentsRect().contains(bar_point)) {
                drag_tab->setWindowTitle(this->tabText(index)); //获取当前tab名字
                popPage(drag_tab);
            }
        });
...
void MyTabWidget::popPage(QWidget* page)
{
    ...
    connect(pop, &MyTabPopup::backToTabWidget, this, [=] {
        QWidget* content = pop->getContentWidget();
        content->setWindowTitle(pop->getContentWidget()->windowTitle());
        this->appendNormalPage(content);
        pop->close();
    });
