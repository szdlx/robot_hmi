...
#include "../include/cyrobot_monitor/videowidget.h"
#include "qdebug.h"
#include "qdir.h"
#include "qevent.h"
    ...
    : QWidget(parent)
{
    ...
    //    QList<QString> video_btns;
    video_btns << "btnFlowName"
               << "btnFlowHor"
               << "btnFlowVer"
               << "btnFlowRotate"; // << "btnFlowClose";
    //    QList<QString> video_names;
    video_names << "video"
                << "水平镜像"
                << "垂直镜像"
                << "0°"; //<< "encode";

    //    QList<bool> video_ischecked;
    video_ischecked << false << false << false << false;
    video_rotate_status << "0°"
                        << "90°"
                        << "180°"
                        << "270°";
    rotate_id = 0;
    ...
    connect(this, SIGNAL(receiveImage(const QImage)), this, SLOT(updateImage(const QImage)));
}
...
void VideoWidget::initFlowPanel()
{
    ...
    //    //按钮集合名称,如果需要新增按钮则在这里增加即可
    //    QList<QString> video_btns;
    //    video_btns << "btnFlowName" << "btnFlowHor" << "btnFlowVer" << "btnFlowRotate";// << "btnFlowClose";
    ...
    //    QList<QString> video_names;
    //    video_names << "video" << "水平镜像" << "垂直镜像" << "0" ;//<< "encode";
    ...
    flowButton = new QButtonGroup;
    //循环添加顶部按钮
    for (int i = 0; i < video_btns.count(); i++) {
        QPushButton* btn = new QPushButton;
        flowButton->addButton(btn, i);
        //绑定按钮单击事件,用来发出信号通知
        connect(btn, SIGNAL(clicked(bool)), this, SLOT(btnClicked()));
        //设置标识,用来区别按钮
        btn->setObjectName(video_btns.at(i));
        //设置固定宽度
        btn->setFixedWidth(this->width() / (video_btns.count() + 1));
        //设置拉伸策略使得填充
        ...
        if (i == 3) {
            switch (rotate_id) {
            case 0:
                btn->setText(video_rotate_status.at(0));
                break;
            case 1:
                btn->setText(video_rotate_status.at(1));
                break;
            case 2:
                btn->setText(video_rotate_status.at(2));
                break;
            case 3:
                btn->setText(video_rotate_status.at(3));
                break;
            }
        } else {
            btn->setText(video_names.at(i));
        }
#endif
    ...
    enbox = new QComboBox;
    QString enText[10] = { "BGR8", "RGB8", "BGR16", "BGRA8", "MONO8", "RGB16", "RGBA8", "BGRA16", "MONO16", "RGBA16" };
    for (int i = 0; i < 10; i++) {
        enbox->addItem(enText[i]);
    }
    enbox->setObjectName("enbox");
    connect(enbox, SIGNAL(currentIndexChanged(int)), this, SLOT(comboxCurrentChange(int)));
...
void VideoWidget::btnClicked()
{
    ...
    for (int i = 0; i < video_btns.count(); i++) {
        if (objName == video_btns[i]) {
            index = i;
            //           flowPressColor = "#5EC7D9";
            //           this->initFlowStyle();
            //           qDebug()<< "flowPressColor :  " << flowPressColor;
        }
    }
    if (index == 3) {
        rotate_id = (rotate_id + 1) % 4;
        btn->setText(video_rotate_status.at(rotate_id));
    } else {
        if (!video_ischecked[index]) {
            video_ischecked[index] = !video_ischecked[index];
            flowPressColor = "#5EC7D9";
            btn->setStyleSheet(QString("QPushButton{color:%1;}").arg(flowPressColor.name()));
            btn->show();
        } else {
            video_ischecked[index] = !video_ischecked[index];
            flowPressColor = "#000000";
            btn->setStyleSheet(QString("QPushButton{color:%1;}").arg(flowPressColor.name()));
            btn->show();
        }
    }

    qDebug() << "test: " << index << " video_ischecked: " << video_ischecked[index];

    emit btnClicked(btn->objectName());
}

void VideoWidget::comboxCurrentChange(int index)
{
    QComboBox* combox = (QComboBox*)sender();
    emit comboxCurrentChange(index, combox->objectName());
    qDebug() << "combox" << index;
}
