...
#include "../include/cyrobot_monitor/videopanel.h"
#include "qevent.h"
#include "qmenu.h"
#include "qlayout.h"
#include "qlabel.h"
...
{
    this->initControl();//设置网格布局
    this->initForm();//设置样式表和每个窗口
    this->initMenu();//添加右键按钮
    this->show_video_all();//根据选中按钮对视频排布
...
void VideoPanel::initForm()
{
    ...
    for (int i = 0; i < videoCount; i++) {
        ...
        for(int j = 0;j < 4; j++){
            ...
            if( j == 3){
                widget->rotate_id = video_btn_setting.value(key_btn_name).toInt();
                widget->flowButton->button(j)->setText(widget->video_rotate_status.at(video_btn_setting.value(key_btn_name).toInt()));
            }
            else{
                widget->video_ischecked[j] = video_btn_setting.value(key_btn_name).toBool();
                if(widget->video_ischecked[j]) {
                    QColor temp_Color = "#5EC7D9";
                    widget->flowButton->button(j)->setStyleSheet(QString("QPushButton{color:%1;}").arg(temp_Color.name()));
                    widget->flowButton->button(j)->show();
                }
                else {
                    QColor temp_Color = "#000000";
                    widget->flowButton->button(j)->setStyleSheet(QString("QPushButton{color:%1;}").arg(temp_Color.name()));
                    widget->flowButton->button(j)->show();
                }
            }
        ...
        widget->setFlowBgColor(Qt::white);      //设置背景颜色
